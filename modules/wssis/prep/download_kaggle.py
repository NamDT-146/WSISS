...
def _run_kaggle(args: list[str], download_dir: Path) -> None:
    ...
    print(f"[download] Initializing Kaggle API for dataset: {args[0]}")
    api = KaggleApi()
    api.authenticate()
    api.dataset_download_files(dataset=args[0], path=str(download_dir), unzip=True, quiet=False)
...
def _organize_coco(extract_root: Path) -> None:
    ...
    if src is None:
        if (extract_root / "annotations").exists():
            ...
            raise FileNotFoundError(f"Could not find coco2017 folder under {extract_root}.")
...
def _organize_minitrain(extract_root: Path) -> None:
    ...
    if (dest / "train2017.txt").exists():
        ...
        return
    candidates = [extract_root / "coco_minitrain_10k", _find_subdir(extract_root, "coco_minitrain_10k")]
    src = next((c for c in candidates if c and c.exists()), None)
    if src is None:
        if (extract_root / "train2017.txt").exists():
            src = extract_root
        else:
            raise FileNotFoundError(f"Could not find coco_minitrain_10k under {extract_root}.")
...
def run(skip_coco: bool = False, skip_minitrain: bool = False) -> None:
    ...
    if not kaggle_json.exists():
        raise FileNotFoundError(f"Place your Kaggle API credentials at {kaggle_json}")
