...
from modules.wssis.pseudo_label_confidence import parse_pseudo_label_config
...
def _pseudo_label_settings_for_ckpt(gnn_ckpt: Path) -> tuple[str, float]:
    """Stage-1 ``pseudo_label`` mode and p_cutoff for Stage-2 YAML."""
    import torch

    if not gnn_ckpt.exists():
        pl = parse_pseudo_label_config(None)
